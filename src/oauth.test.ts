...
test("the app id defaults to the A1-x-Tech public client and asks only for the partner scope", () => {
  const saved = process.env.YANDEX_MERCHANTS_OAUTH_CLIENT_ID;
  delete process.env.YANDEX_MERCHANTS_OAUTH_CLIENT_ID;
  try {
    assert.equal(oauthClientId(), DEFAULT_CLIENT_ID);
    assert.equal(DEFAULT_CLIENT_ID, "9e20243a34854a6f84156062be9cccf8");
    assert.equal(SCOPE, "products:partner-api");
  } finally {
    if (saved !== undefined) process.env.YANDEX_MERCHANTS_OAUTH_CLIENT_ID = saved;
  }
});
 ...
 * live endpoint (by the sibling Metrica server). Handling only the spec's
 * `invalid_grant` sent the most common failure of the whole flow (the code
 * expires in 10 minutes) to the generic branch, which relays "Invalid code"
 * without saying a fresh one is needed.
 */
