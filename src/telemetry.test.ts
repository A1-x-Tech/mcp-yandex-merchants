...
test("opt-out sends nothing", () => {
  ...
  t.send("server_start");
  t.send("tool_call", { tool: "list_feeds" });
  assert.equal(sent.length, 0);
});

test("unconfigured_start carries the reason code from the closed vocabulary", () => {
  const sent: Sent[] = [];
...
test("startup_failed stays a valid event for a config unusable at load time", () => {
  const sent: Sent[] = [];
  new Telemetry("1.0.0", true, recordingFetch(sent)).send("startup_failed", {});
  const [ping] = sent;
...
test("server_start carries clientInfo from the MCP handshake", async () => {
  ...
  assert.equal(start.body.app, "mcp-yandex-merchants");
});
