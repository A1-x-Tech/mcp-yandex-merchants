 ...
 * still means "config unusable at load time" (malformed values — no such
 * checks exist today); it is kept unchanged so the historical funnel stays
 * comparable. `unconfigured_start` is its live counterpart — the server now
 * survives a missing token and completes the handshake, so this is the first
 * event that can carry the client's name for an install with no token yet.
 */
...
export class Telemetry {
   ...
   * no startup path uses it anymore — the server survives a bad config, so
