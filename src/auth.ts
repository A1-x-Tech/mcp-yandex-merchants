 ...
 * it names the fix instead of the failure. Replaces the former CredentialsError
 * — one class, one text, now with two fixes: the in-chat login and the env var.
 */
...
export const NOT_CONNECTED_MESSAGE =
  "Яндекс Товары не подключены: нет OAuth-токена. " +
  ...
  "под тем логином Яндекса, под которым загружен YML-фид, и прислать код подтверждения, " +
  "затем передайте этот код в finish_login. " +
  "Альтернатива без диалога — задать переменную окружения YANDEX_MERCHANTS_OAUTH_TOKEN " +
  "(токен со scope products:partner-api) в конфигурации MCP-клиента и перезапустить сервер.";

/**
 * Resolves the access token for every request. Two sources, in this order:
 *
 *   env    — YANDEX_MERCHANTS_OAUTH_TOKEN, the documented setup; never touched or refreshed
 *   stored — ~/.config/mcp-yandex-merchants/credentials.json, written by finish_login
 *
...
export class TokenStore {
  ...
  async getToken(): Promise<string> {
    ...
    if (!stored.refresh_token) {
      throw new AuthRequiredError(
        "Срок действия сохранённого токена Яндекс Товаров истёк, а обновить его нечем. " +
  ...
  async refresh(refreshToken?: string): Promise<StoredCredentials> {
    ...
    // Yandex may answer a refresh without a new refresh_token. Saving that
    // verbatim would erase the stored one and strand the user at the next
    // expiry — keep the token that just worked unless a rotated one arrived.
