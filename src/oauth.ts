...
/**
 * OAuth app «Яндекс Товары — A1-x-Tech MCP», scoped to `products:partner-api`
 * only. A public PKCE client — the id is not a secret. Overridable so a user
 * who wants their own app (own consent screen) can point the flow at it
 * without forking.
 */
export const DEFAULT_CLIENT_ID = "9e20243a34854a6f84156062be9cccf8";
...
/**
 * Requested rights. Sent explicitly even though the app is already scoped to
 * the Merchants partner API: without `scope` Yandex issues a token carrying
 * **every** right the app holds, so an app that gains a right later would
 * silently widen every token minted afterwards.
 */
export const SCOPE = "products:partner-api";
...
export function oauthClientId(): string {
  return process.env.YANDEX_MERCHANTS_OAUTH_CLIENT_ID || DEFAULT_CLIENT_ID;
}
...
/**
 * No `state` parameter: state exists to tie a redirect back to the session that
 * started it, and this flow has no redirect — the user hands the code back by
 * pasting it into the chat, and PKCE already binds that code to this process's
 * verifier. A state nobody ever checks would only look like a security measure.
 */
  ...
  switch (code) {
    ...
    case "invalid_client":
      return (
        ...
        "Если задан YANDEX_MERCHANTS_OAUTH_CLIENT_ID — проверьте его; " +
