...
async function withTempConfig<T>(run: () => T | Promise<T>): Promise<T> {
  ...
  process.env.XDG_CONFIG_HOME = mkdtempSync(join(tmpdir(), "mcp-merchants-test-"));
  try {
    return await run();
  } finally {
...
test("with nothing configured, getToken explains how to connect — both fixes", async () => {
  await withTempConfig(async () => {
    ...
    // name both fixes — the in-chat login and the env var.
    await assert.rejects(() => store.getToken(), (err: Error) => {
      ...
      assert.match(err.message, /finish_login/);
      assert.match(err.message, /YANDEX_MERCHANTS_OAUTH_TOKEN/);
      assert.match(err.message, /перезапустить сервер/);
      assert.match(err.message, /логином Яндекса, под которым загружен YML-фид/);
      assert.match(err.message, /products:partner-api/);
      return true;
    });
  });
});
...
test("saving leaves exactly one file behind — no temp leftovers from the atomic write", async () => {
  await withTempConfig(() => {
    // Twice: the second write goes over an existing file, the branch where a
    // non-atomic truncate-in-place used to be the risk.
...
test("a refresh answer without a refresh_token keeps the stored one", async () => {
  await withTempConfig(async () => {
    ...
    // Yandex is allowed to omit refresh_token on refresh; persisting that
    // verbatim would strand the user at the next expiry.
    const fetchImpl = (async () =>
      ({
        ok: true,
        status: 200,
...
test("logout does not touch the env token path", async () => {
  await withTempConfig(async () => {
    writeCredentials({ access_token: "stored", obtained_at: Date.now() });
    const store = new TokenStore("from-env");
    assert.equal(store.logout(), true, "the stored file is removed");
    // The env token stays: it lives in the client's config, not on this disk.
    assert.equal(await store.getToken(), "from-env");
    assert.equal(store.status().source, "env");
  });
});
